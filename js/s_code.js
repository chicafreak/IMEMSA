...
if(!s_account)
{
	var s_account="yamahausamotor" //default s_account if nothing is passed from the page
...
{
	s.linkInternalFilters = 'javascript:,yamaha-motor.com,starmotorcycles.com,localhost'; //,default s.linkInternalFilters if nothing is passed from the page
}
else
{
...
s.visitorNamespace="yamahamotorusa"
s.trackingServer="yamahamotorusa.112.2o7.net"

//s.dynamicAccountSelection=true
//s.dynamicAccountList="daviddev=localhost;almdev,almglobaldev=dev.law.com"
...
s._channelDomain="Social Networks:Natural|facebook.com,myspace.com,twitter.com,youtube.com>Partners|partner1.com,partner2.com"
...
s._channelPattern="Paid Search|knc-,ps->Banner Ad|bac-,ba->eMail|em-,emc->Vanity Redirect|va->Paid Social Media|psm->Affiliates|affil->Tagged Social Media|tsm-"
/* 
Marketing Channel		Begin campaign value with:		example
 
Paid Search				knc- or ps-						knc-1101-google_abc
Banner Ad				bac- or ba-						ba-1105-bannerID
Email					emc- or em-						emc-110115-newletter
Vanity Redirect			va-								va-1101-redirect
Paid Social Media		psm-							psm-1101-facebook_abc
Affiliates				affil-							affil-myAffilate
Tagged Social Media		tsm-							tsm-110101-facebookshare
...
if(typeof $ != "undefined"){
	if($('#hPrintPDF')){
		$('#hPrintPDF').click(function(){
			s.linkTrackVars = s.apl(s.linkTrackVars,"prop40,eVar40,events",",",2);
			s.linkTrackEvents = s.apl(s.events,"event6",",",2);
			s.manualDownload = true;
			var filename = s.pageName+":specifications:pdf spec";
			console.log($(this).attr("href"),filename);
...
function s_doPlugins(s) {
	...
	///* Automatically Capture Exit Links */
		...
		if(s.exitURL)
		{
			...
			s.linkTrackVars = s.apl(s.linkTrackVars,"prop5,prop6,prop7,prop8",",",2);
	...
	/* External Campaign Tracking */
		if(!s.campaign)
		{
			s.campaign=s.getQueryParam('rss,sharecode,affil,cmp,cmpid');
		...
		/* Campaign Click Past */
		s.clickPast(s.campaign,'event3','event31');
	...
	/* normalize page name and channel variables */
		if(s.pageName)
		{
		...
		if(s.channel)
		{
			s.channel = s.channel.split('+').join(' ')
			s.channel = s.channel.split('/').join(':');	
		}
		
		if (!s.server || s.server.length < 2) { s.server = 'unknown' }
		if (!s.channel || s.channel.length < 2) { s.channel = 'unknown' }
		
		if (s.server == 'YamahaConsumer' || s.server == 'yamahaconsumer') { s.server = 'yamaha' }
		if (s.server == 'StarConsumer' || s.server == 'starconsumer') { s.server = 'star' }	
		
		if (s.server.substring(s.server.length-1) == ':') { s.server = s.server.substring(0, s.server.length-1); }
		if (s.channel.substring(s.channel.length-1) == ':') { s.channel = s.channel.substring(0, s.channel.length-1); }
		if(s.loadPage)
		{
			s.channel = s.server + ':' + s.channel.split('::').join(':');
			if(!s.pageType){ s.pageName = s.channel + ':' + s.pageName.split('::').join(':'); }
			s.loadPage = false;
		}		
		s.server = s.server.split('::').join(':');
		s.channel = s.channel.split('::').join(':');
		s.pageName = s.pageName.split('::').join(':');
		
	/* SET DETAIL CONTENT METRICS -- products */
		if(s.pageName && s.pageName.indexOf('products') > -1)
		{
					s.prop11 = 'no id';
					s.prop11 = s.getValOnce(s.prop11.toLowerCase(),'s_prop11',0)
					s.eVar11 = "D=c11";
					s.prop13 = s.channelExtractCust(":",4,4,s.pageName,1);
					s.prop13 = s.getValOnce(s.prop13.toLowerCase(),'s_prop13',0)
					s.eVar13 = "D=c13";
					s.prop14 = s.channelExtractCust(":",5,5,s.pageName,1);
					s.prop14 = s.getValOnce(s.prop14.toLowerCase(),'s_prop14',0)
					s.eVar14 = "D=c14";
					s.prop15 = s.channelExtractCust(":",6,6,s.pageName,1);
					s.prop15 = s.getValOnce(s.prop15.toLowerCase(),'s_prop15',0)
					s.eVar15 = "D=c15";
					s.prop16 = s.channelExtractCust(":",7,7,s.pageName,1);
					s.prop16 = s.getValOnce(s.prop16.toLowerCase(),'s_prop16',0)
					s.eVar16 = "D=c16";
					//s.prop17 = s.channelExtractCust(":",8,8,s.pageName,1);
//					s.prop17 = s.getValOnce(s.prop17.toLowerCase(),'s_prop17',0)
//					s.eVar17 = "D=c17";
					if(s.prop11 || s.prop13 || s.prop14 || s.prop15 || s.prop16)
					{
						s.events = s.apl(s.events,'event2',',',2)
					}
		}
	/* SET DETAIL CONTENT METRICS -- accessories */
		if(s.pageName && s.pageName.indexOf('apparel') > -1)
		{
					s.prop11 = 'no id';
					s.prop11 = s.getValOnce(s.prop11.toLowerCase(),'s_prop11',0)
					s.eVar11 = "D=c11";
					...
					s.prop13 = s.getValOnce(s.prop13.toLowerCase(),'s_prop13',0)
					s.eVar13 = "D=c13";
					s.prop14 = s.channelExtractCust(":",4,4,s.pageName,1);
					s.prop14 = s.getValOnce(s.prop14.toLowerCase(),'s_prop14',0)
					s.eVar14 = "D=c14";
					s.prop15 = 'no year';
					s.prop15 = s.getValOnce(s.prop15.toLowerCase(),'s_prop15',0)
					...
					s.prop16 = s.getValOnce(s.prop16.toLowerCase(),'s_prop16',0)
					s.eVar16 = "D=c16";
					//s.prop17 = s.channelExtractCust(":",5,5,s.pageName,1);
//					s.prop17 = s.getValOnce(s.prop17.toLowerCase(),'s_prop17',0)
//					s.eVar17 = "D=c17";
					if(s.prop11 || s.prop13 || s.prop14 || s.prop15 || s.prop16)
					{
	...
	/* SET DETAIL CONTENT METRICS -- accessories */
		if(s.pageName && s.pageName.indexOf('news:articles') > -1)
		{
					s.prop11 = 'no id';
					s.prop11 = s.getValOnce(s.prop11.toLowerCase(),'s_prop11',0)
					s.eVar11 = "D=c11";
					...
					s.prop13 = s.getValOnce(s.prop13.toLowerCase(),'s_prop13',0)
					s.eVar13 = "D=c13";
					s.prop14 = s.channelExtractCust(":",3,4,s.pageName,1);
					s.prop14 = s.getValOnce(s.prop14.toLowerCase(),'s_prop14',0)
					s.eVar14 = "D=c14";
					s.prop15 = 'no year';
					s.prop15 = s.getValOnce(s.prop15.toLowerCase(),'s_prop15',0)
...
//					s.eVar17 = "D=c17";
		...
		if(s.pageName)
		{
			...
			if(!s.prop1)
			{
				s.prop1=s.channelExtractCust(":",3,4,s.channel,1);
				s.eVar1 = "D=c1";
			}
			if(!s.prop2)
			{
				s.prop2=s.channelExtractCust(":",3,5,s.channel,1);
				s.eVar2 = "D=c2";
			}
			if(!s.prop3)
			{
				s.prop3=s.channelExtractCust(":",3,6,s.channel,1);
				s.eVar3 = "D=c3";
			}
			if(!s.prop4)
			{
				s.prop4=s.channelExtractCust(":",3,10,s.channel,1);
				s.eVar4 = "D=c4";
			}
			if(!s.prop12)
			{
				s.prop12=s.channelExtractCust(":",3,3,s.pageName,1);
				s.eVar12 = "D=c12";
			}
		}
		if(s.channel)
	...
	/* set LifeStyle */
		if(!s.prop43)
		{
			s.prop43 = s.channelExtractCust(':',2,2,s.pageName,1);
			s.eVar43 = "D=c43";
		}
	
	 	/* Plugin Example: setupLinkTrack 0.1 */
		if(s.manualDownload==false){
	...
	/* Site Search */
		if(s.prop31)
		{
			s.prop31 = s.getValOnce(s.prop31,'search_prop31',0);
			if(s.prop31)
			{
				s.events=s.apl(s.events,"event21",",",2);
				s.eVar31='D=c31';
				s.prop37 = s.getPreviousValue(s.pageName,'qpv_pagename','');
				if(s.prop37)
				{
					s.eVar37 = "D=c37";
				}
	...
	/* Internal Search Refinements */
		if(s.prop32)
		{
			s.prop32 = s.getValOnce(s.prop32,'search_prop32',0);
			if(s.prop32)
			{
	...
	/* Set Channel Variables */
		s.channelManager('rss,sharecode,src,affil,cmp,cmpid');
		if(s._channel)
		{
			s.prop46 = s._channel;
			s.eVar46 = "D=c46";
			...
			{
				s.prop47 = s.eVar47 = 'n/a';
			}
			if(s._keywords)
			{
				s.prop48 = s._keywords;
				s.eVar48 = "D=c48";
			}
			else
			{
				s.prop48 = s.eVar48 = 'n/a';
			}			
			if(s._referringDomain)
			{
				s.prop49 = s._referringDomain;
				s.eVar49 = "D=c49";
			}
			else
			{
				s.prop49 = s.eVar49 = 'n/a';
			}
	...
	/* Membership Tracking*/
	s.prop19=s.getAndPersistValue(s.prop19,'s_prop19_persist',0);
	s.prop20=s.getAndPersistValue(s.prop20,'s_prop20_persist',0);
	if(s.prop19)
		{
			s.eVar19 = s.prop19;
			s.eVar19 = s.getValOnce(s.eVar19,'s_evar19',0);
			if(s.eVar19)
			{
		...
		//Delimited values placed into additional variables
					if (s.pageName){
					var pageNameArray=s.pageName.split(":");
					s.prop61=pageNameArray[0];
					if(pageNameArray[1]){
					s.prop61=pageNameArray[0];
					s.prop62=pageNameArray[1];
					}
					if(pageNameArray[2]){
					s.prop61=pageNameArray[0];
					s.prop62=pageNameArray[1];
					s.prop63=pageNameArray[2];
					}
					if(pageNameArray.length>=3){
					s.prop61=pageNameArray[0];
					s.prop62=pageNameArray[1];
					s.prop63=pageNameArray[2];
					s.prop64=pageNameArray[3];
					}
					if(pageNameArray.length>=4){
					s.prop61=pageNameArray[0];
					s.prop62=pageNameArray[1];
					s.prop63=pageNameArray[2];
					s.prop64=pageNameArray[3];
					s.prop65=pageNameArray[4];
					}
					...
					s.prop65=pageNameArray[4];
					s.prop66=pageNameArray[5];
					s.prop67=pageNameArray[6];
					}
					if(pageNameArray.length>=7){
					s.prop61=pageNameArray[0];
					s.prop62=pageNameArray[1];
					s.prop63=pageNameArray[2];
					s.prop64=pageNameArray[3];
					...
					s.prop66=pageNameArray[5];
					s.prop67=pageNameArray[6];
					s.prop68=pageNameArray[7];
					}
					if(pageNameArray.length>=8){
					s.prop61=pageNameArray[0];
					s.prop62=pageNameArray[1];
					s.prop63=pageNameArray[2];
					s.prop64=pageNameArray[3];
					...
					s.prop67=pageNameArray[6];
					s.prop68=pageNameArray[7];
					s.prop69=pageNameArray[8];
					}
					if(pageNameArray.length>=9){
					s.prop61=pageNameArray[0];
					s.prop62=pageNameArray[1];
					s.prop63=pageNameArray[2];
					s.prop64=pageNameArray[3];
